...
// Basic session check
if (!isset($_SESSION['center_admin_logged_in']) || $_SESSION['center_admin_logged_in'] !== true) {
    header("Location: center_login.php");
    exit;
}

// Verify session token
$stmt = $conn->prepare("SELECT session_token FROM center_details WHERE id = ?");
...
if ($db_token !== $_SESSION['session_token']) {
    session_unset();
    session_destroy();
    header('Location: center_login.php?error=session_expired');
    exit;
}
...
$stmt = $conn->prepare("SELECT COUNT(*) FROM students WHERE study_center = ?");
...
$stmt = $conn->prepare("SELECT COUNT(DISTINCT course_name) FROM courses");
$stmt->execute();
...
$stmt = $conn->prepare("
    ...
    WHERE c.approved = 1 AND s.study_center = ?
");
$stmt->bind_param("s", $center_name);
$stmt->execute();
$stmt->bind_result($certificate_count);
$stmt->fetch();
$stmt->close();

$center_name = $_SESSION['center_name'];

$stmt = $conn->prepare("SELECT SUM(price) FROM students WHERE study_center = ?");
$stmt->bind_param("s", $center_name);
$stmt->execute();
...
// Format if null (i.e. no students)
								...
								<div class="row">
									...
									<div class="col-xl-3 col-6">
										<div class="content-box">
											...
											<div class="chart-num">
												<p>Certifaction</p>
												<h2 class="font-w700 mb-0"><?php echo $certificate_count; ?></h2>
											</div>
										</div>
									</div>
									<div class="col-xl-3 col-6">
										<div class="content-box">
											...
											<div class="chart-num">
												<p>Courses</p>
												<h2 class="font-w700 mb-0"><?php echo $courses_count; ?></h2>
											</div>
										</div>
									</div>
									<div class="col-xl-3 col-6">
